...
function NewOrderDialog({ open, onOpenChange, onSaved, children }: TwoStepOrderDialogProps) {
  ...
  const getCategoryEmoji = (categoria?: string) => {
    ...
    if (cat.includes("cerveja")) return "🍺"
  ...
  return (
    <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
      ...
      <DialogContent className="w-[95vw] max-w-2xl h-[90vh] max-h-[90vh] mx-auto overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto px-1">
          <div className="text-center py-8 text-gray-500">Sistema de pedidos otimizado carregando...</div>
        </div>

        <div className="flex-shrink-0 flex justify-between items-center pt-4 border-t border-gray-600 bg-gray-900">
          <Button
            variant="outline"
            className="border-gray-500 text-gray-300 hover:bg-gray-700 hover:border-gray-400 transition-colors bg-transparent"
          >
            <ArrowLeft className="mr-2 h-4 w-4" />
            Produtos
          </Button>
